/*!
 * @file PhaserPreferences.cpp
 * @brief Phaser Prow Enhanced light gun preferences to save in non-volatile memory.
 *
 * @copyright Fusioon Dev 2022
...
#include <EEPROM.h>
#endif // Phaser_EEPROM_ENABLE

// 4 byte header ID
const PhaserPreferences::HeaderId_t PhaserPreferences::HeaderId = {'P', 'r', 'o', 'w'};

#if defined(Phaser_FLASH_ENABLE)

// must match Errors_e order
const char* PhaserPreferences::ErrorText[6] = {
    "Success",
    "No storage memory",
    "Read error",
...
const char* PhaserPreferences::ErrorCodeToString(int error)
{
    if(error >= 0) {
...
int PhaserPreferences::Load(Adafruit_SPIFlashBase& flash)
{
    uint32_t u32;
...
int PhaserPreferences::Save(Adafruit_SPIFlashBase& flash)
{
...
#elif defined(Phaser_EEPROM_ENABLE)

int PhaserPreferences::Load()
{
    uint32_t u32;
    EEPROM.get(0, u32);
    if(u32 != HeaderId.u32) {
