/*!
 * @file Phaser_Pro_Micro.ino
 * @brief Arduino Powered Light Gun sketch for 4 IR LED setup.
 * Based on the 4IR Beta "Big Code Update" SAMCO & Prow7 enchancemnets
 *
 * @copyright Fusion Lightguns, https://github.com/Fusion-Lightguns/Fusion-Phaser/blob/main/LICENSE, Dec 2022
...
#endif
#ifdef PHASER_FLASH_ENABLE
#include <Adafruit_SPIFlashBase.h>
#endif // PHASER_FLASH_ENABLE
#include <AbsMouse5.h>
#include <DFRobotIRPositionEx.h>
#include <LightgunButtons.h>
#include <PhaserPositionEnhanced.h>
#include <PhaserConst.h>
#include "PhaserColours.h"
#include "PhaserPreferences.h"
...
const LightgunButtons::Desc_t LightgunButtons::ButtonDesc[] = {
    {4, LightgunButtons::ReportType_Mouse, MOUSE_LEFT, 20, BTN_AG_MASK, "Trigger"},
    {A0, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, 20, BTN_AG_MASK2, "A"},
    {A1, LightgunButtons::ReportType_Mouse, MOUSE_MIDDLE, 20, BTN_AG_MASK2, "B"},
    ...
    {6, LightgunButtons::ReportType_Keyboard, KEY_UP_ARROW, 25, BTN_AG_MASK2, "Up"},
    {7, LightgunButtons::ReportType_Keyboard, KEY_DOWN_ARROW, 25, BTN_AG_MASK2, "Down"},
    {8, LightgunButtons::ReportType_Keyboard, KEY_LEFT_ARROW, 25, BTN_AG_MASK2, "Left"},
    {9, LightgunButtons::ReportType_Keyboard, KEY_RIGHT_ARROW, 25, BTN_AG_MASK2, "Right"},
    {15, LightgunButtons::ReportType_Internal, MOUSE_BUTTON4, 20, BTN_AG_MASK2, "Reload"},
    {5, LightgunButtons::ReportType_Mouse, MOUSE_RIGHT, 20, BTN_AG_MASK2, "Pedal"}
};
...
// if you have original Phaser calibration values, multiply by 4 for the center position and
// scale is multiplied by 1000 and stored as an unsigned integer, see PhaserPreferences::Calibration_t
PhaserPreferences::ProfileData_t profileData[ProfileCount] = {
...
// Phaser positioning
PhaserPositionEnhanced myPhaser;
...
PhaserPreferences::Preferences_t PhaserPreferences::preferences = {
...
int nvPrefsError = PhaserPreferences::Error_NoStorage;

// preferences instance
PhaserPreferences phaserPreferences;

// number of times the IR camera will update per second
constexpr unsigned int IRCamUpdateRate = 209;

#ifdef PHASER_NO_HW_TIMER
...
#define PHASER_NO_HW_TIMER_UPDATE() NoHardwareTimerCamTickMillis()
//define PHASER_NO_HW_TIMER_UPDATE() NoHardwareTimerCamTickMicros()

#else
#define PHASER_NO_HW_TIMER_UPDATE()
// timer will set this to 1 when the IR position can update
volatile unsigned int irPosUpdateTick = 0;
#endif // PHASER_NO_HW_TIMER
...
#if defined(PHASER_SAMD21) || defined(PHASER_SAMD51)
...
void ExecRunMode()
{
    ...
    for(;;) {
        ...
        if(irPosUpdateTick) {
            ...
            int halfHscale = (int)(myPhaser.h() * xScale + 0.5f) / 2;
            moveXAxis = map(finalX, xCenter + halfHscale, xCenter - halfHscale, 0, MouseMaxX);
            halfHscale = (int)(myPhaser.h() * yScale + 0.5f) / 2;
    ...
    for(;;) {
        ...
        if(irPosUpdateTick) {
            ...
            if(error == DFRobotIRPositionEx::Error_Success) {
                myPhaser.begin(dfrIRPos.xPositions(), dfrIRPos.yPositions(), dfrIRPos.seen(), MouseMaxX / 2, MouseMaxY / 2);
                UpdateLastSeen();
                for(int i = 0; i < 4; i++) {
                    Serial.print(map(myPhaser.testX(i), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
                    Serial.print(",");
                    Serial.print(map(myPhaser.testY(i), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
                    Serial.print(",");
                }
                Serial.print(map(myPhaser.x(), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
                Serial.print(",");
                Serial.print(map(myPhaser.y(), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
                Serial.print(",");
                Serial.print(map(myPhaser.testMedianX(), 0, MouseMaxX, CamMaxX, 0) + processingOffset);
                Serial.print(",");
                Serial.println(map(myPhaser.testMedianY(), 0, MouseMaxY, CamMaxY, 0) + processingOffset);
...
{
    if(GetPositionIfReady()) {
        int halfH = (int)(myPhaser.h() * yScale + 0.5f) / 2;
...
{
    if(GetPositionIfReady()) {    
        int halfH = (int)(myPhaser.h() * xScale + 0.5f) / 2;
...
void GetPosition()
{
    int error = dfrIRPos.basicAtomic(DFRobotIRPositionEx::Retry_2);
    if(error == DFRobotIRPositionEx::Error_Success) {
        myPhaser.begin(dfrIRPos.xPositions(), dfrIRPos.yPositions(), dfrIRPos.seen(), xCenter, yCenter);
#ifdef EXTRA_POS_GLITCH_FILTER
        if((abs(myPhaser.X() - finalX) > BadMoveThreshold || abs(myPhaser.Y() - finalY) > BadMoveThreshold) && badFinalTick < BadMoveCountThreshold) {
            ++badFinalTick;
        } else {
            if(badFinalTick) {
                badFinalCount++;
...
void UpdateLastSeen() {
    if(lastSeen != myPhaser.seen()) {
        if(!lastSeen && myPhaser.seen()) {
            LedOff();
        } else if(lastSeen && !myPhaser.seen()) {
            SetLedPackedColor(IRSeen0Color);
        }
        lastSeen = myPhaser.seen();
    }
}
...
void PrintPreferences()
{
    ...
    Serial.println(profileDesc[PhaserPreferences::preferences.profile].profileLabel);
    
    Serial.println("Profiles:");
    for(unsigned int i = 0; i < PhaserPreferences::preferences.profileCount; ++i) {
...
{
#ifdef PHASER_FLASH_ENABLE
    unsigned int required = PhaserPreferences::Size();
    ...
    Serial.print((unsigned int)sizeof(PhaserPreferences::ProfileData_t));
...
{
    if(nvPrefsError != PhaserPreferences::Error_Success) {
        Serial.print(NVRAMlabel);
        Serial.print(" error: ");
#ifdef PHASER_FLASH_ENABLE
        Serial.println(PhaserPreferences::ErrorCodeToString(nvPrefsError));
#else
...
#ifdef PHASER_FLASH_ENABLE
    nvPrefsError = PhaserPreferences::Load(flash);
#else
    nvPrefsError = phaserPreferences.Load();
...
void VerifyPreferences()
{
    ...
    if(PhaserPreferences::preferences.profile >= ProfileCount) {
        PhaserPreferences::preferences.profile = (uint8_t)selectedProfile;
    }
}
...
{   
    ...
    if(PhaserPreferences::preferences.profile < ProfileCount) {
        ...
        selectedProfile = PhaserPreferences::preferences.profile;
...
#ifdef PHASER_FLASH_ENABLE
    nvPrefsError = PhaserPreferences::Save(flash);
#else
    nvPrefsError = PhaserPreferences::Save();
#endif // PHASER_FLASH_ENABLE
    if(nvPrefsError == PhaserPreferences::Error_Success) {
