/*
    ...
    // modified for use with Phaser light guns 4 Led setup by Sam Ballantyne April 2020.
...
myDFRobotIRPosition.requestPosition();
    ...
    for (int i = 0; i < 4; i++) {
      rawX[i] = myPhaser.testX(i);
      rawY[i] = myPhaser.testY(i);
    }
    mouseX = myPhaser.X();
    mouseY = myPhaser.Y();
    medianX = myPhaser.testMedianX();
    medianY = myPhaser.testMedianY();
    time = micros();
    }
    else {
...
myDFRobotIRPosition.requestPosition();
    ...
    myPhaser.begin(myDFRobotIRPosition.readX(0), myDFRobotIRPosition.readY(0), myDFRobotIRPosition.readX(1), myDFRobotIRPosition.readY(1),myDFRobotIRPosition.readX(2), myDFRobotIRPosition.readY(2),myDFRobotIRPosition.readX(3), myDFRobotIRPosition.readY(3), xCenter, yCenter);
    for (int i = 0; i < 4; i++) {
      rawX[i] = map(myPhaser.testX(i), 0, 1023, 1023, 0);
      rawY[i] = map(myPhaser.testY(i), 0, 768, 768, 0);
    }
    mouseX = map(myPhaser.X(), 0, 1023, 1023, 0);
    mouseY = map(myPhaser.Y(), 0, 768, 768, 0);
    centerX = map(myPhaser.testMedianX(), 0, 1023, 1023, 0);
    centerY = map(myPhaser.testMedianY(), 0, 768, 768, 0);
    }
    else {
    Serial.println("Device not available!");
    }
